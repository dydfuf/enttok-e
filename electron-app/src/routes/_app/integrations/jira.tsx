...
import {
	fetchJiraDebugSearch,
	type JiraDebugResponse,
} from "@/lib/atlassian-api";
import type { ActivityEvent } from "@/lib/activity-api";

export const Route = createFileRoute("/_app/integrations/jira")({
...
function JiraIntegrationPage() {
	...
	const [devResult, setDevResult] = useState<JiraDebugResponse | null>(null);
	...
	const handleSync = async (accountId: string) => {
		...
		try {
			...
			setSyncError(
				err instanceof Error ? err.message : "Failed to sync Jira account",
			);
		} finally {
	...
	const handleDevFetch = async () => {
		if (!backendState?.port || !backendState?.token || !devAccountId) {
			setDevError("Backend not ready or account not selected.");
			return;
		}
		setDevLoading(true);
		setDevError(null);
		try {
			const data = await fetchJiraDebugSearch(
				backendState.port,
				backendState.token,
				devAccountId,
			);
			...
			setDevResult(null);
			setDevError(
				err instanceof Error ? err.message : "Failed to fetch debug response",
			);
		} finally {
			setDevLoading(false);
		}
	...
	useEffect(() => {
		...
		const exists = accounts.some(
			(account) => account.account_id === devAccountId,
		);
		if (!exists) {
			setDevAccountId(accounts[0].account_id);
		}
	}, [accounts, devAccountId]);
				...
				<div className="max-w-3xl mx-auto">
					<div className="mb-6">
						...
						<div className="flex items-center gap-3">
							<SquareKanban className="w-8 h-8 text-gray-900 dark:text-gray-100" />
		...
		<div className="min-h-full p-6">
			<div className="max-w-3xl mx-auto">
							...
							<DialogContent>
								...
								<div className="space-y-4 mt-4">
									<div className="space-y-2">
										<Label htmlFor="jira-org">Org</Label>
										<Input
											id="jira-org"
				...
				<div className="space-y-6 mb-6">
					...
					<DevDebugCard
						accounts={accounts}
						selectedAccountId={devAccountId}
						onSelectAccountId={(value) => {
							setDevAccountId(value);
							setDevError(null);
							setDevResult(null);
						}}
						onFetch={handleDevFetch}
					...
					<div className="space-y-6">
						<div className="p-4 bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-lg">
							...
							<p className="text-sm text-gray-600 dark:text-gray-400 mb-4">
								Connect Jira to see issue activity in your stream.
							</p>
						</div>
						...
						<div className="p-4 bg-gray-50 dark:bg-gray-900/50 border border-gray-200 dark:border-gray-800 rounded-lg">
							...
							<ul className="text-sm text-gray-600 dark:text-gray-400 space-y-1">
								<li>- Issues you create</li>
								<li>- Status changes you make</li>
								<li>- Comments you add</li>
								<li>- Assigned issues and related updates</li>
							</ul>
						</div>
					</div>
				) : (
								...
								<Card key={account.account_id}>
									<CardHeader className="pb-3">
										<div className="flex items-center gap-4">
											<SquareKanban className="w-8 h-8 text-muted-foreground" />
												...
												<AlertDialog>
													...
													<AlertDialogContent>
														<AlertDialogHeader>
															...
															<AlertDialogDescription>
																This will disconnect Jira and remove synced
																activity data.
															</AlertDialogDescription>
														</AlertDialogHeader>
														<AlertDialogFooter>
			...
			<CardContent className="space-y-4">
				<div className="flex items-center justify-between">
					<div className="flex items-center gap-2">
						...
						<span className="text-sm">Jira Account</span>
					</div>
					{connected ? (
						<div className="flex items-center gap-2">
...
type DevDebugCardProps = {
	accounts: { account_id: string; org: string }[];
	selectedAccountId: string;
	onSelectAccountId: (value: string) => void;
	onFetch: () => void;
	isLoading: boolean;
	error: string | null;
	result: JiraDebugResponse | null;
};
...
function DevDebugCard({
	...
	return (
		<Card>
			<CardHeader>
				<CardTitle className="text-base">/dev</CardTitle>
				<CardDescription>
					Inspect the raw Jira API response used by the backend.
				</CardDescription>
			</CardHeader>
			<CardContent className="space-y-4">
				{!hasAccounts ? (
					<p className="text-sm text-muted-foreground">
						Connect a Jira account to enable debug mode.
					</p>
				) : (
					<div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
						<div className="space-y-2">
							<Label htmlFor="jira-dev-account" className="text-xs">
								Account
							</Label>
							<Select value={selectedAccountId} onValueChange={onSelectAccountId}>
								<SelectTrigger id="jira-dev-account" className="w-full sm:w-64">
				...
				{result ? (
					<pre className="max-h-80 overflow-auto rounded-md bg-muted px-3 py-2 text-xs text-muted-foreground">
						{JSON.stringify(result, null, 2)}
					</pre>
				) : (
					hasAccounts && (
						<p className="text-sm text-muted-foreground">
							No debug response fetched yet.
						</p>
					)
				)}
			</CardContent>
